...
namespace FoodDeliveryAPI.Controllers
{
    public class ShopItemTypesController : BaseApiController
    {
        ...
        public async Task<IActionResult> AddItemType([FromForm] CreateItemTypeDto itemType)
        {
            return Ok(await Mediator.Send(new AddItemType.Command { ItemTypeDto = itemType }));
        }
        [HttpPut]
        //[Authorize(Roles = UserRoles.Admin)]
        public async Task<IActionResult> EditItemType([FromForm] EditItemTypeDto itemType)
        {
